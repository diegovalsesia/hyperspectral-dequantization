...
if param.G_load == '' and param.start_iter > 1:
	...
	print('Resuming from %d' % param.start_iter)
...
xQ_test = torch.FloatTensor(config_test.batch_size, config.patch_size[0], config.patch_size[1], config.patch_size[2])
...
## Fitting model
for i in range(param.start_iter, param.start_iter+config.N_iter):
	...
	if i % config.test_every_iter == 0:
		...
		for shift_z in range(0,config_test.patch_size[2], config_test.patch_size[2]):
			...
			for b in range(0,bad_data_test.shape[0],config_test.batch_size):
				...
				if param.cuda:
					xQ_test.data = torch.from_numpy(bad_data_test[b:(b+config_test.batch_size),:,:,:].astype(np.float32)).cuda()
				else:
					xQ_test.data = torch.from_numpy(bad_data_test[b:(b+config_test.batch_size),:,:,:].astype(np.float32))

				x_fake_test = G.eval()(xQ_test)
				
				if b==0:
					x_dq = x_fake_test.cpu().data.numpy()
				else:	
					x_dq = np.concatenate( (x_dq, x_fake_test.cpu().data.numpy()), axis=0 )
